...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Forms /</span> Edit Balita</h4>

        <div class="card mb-4">
            <h5 class="card-header">Edit Balita</h5>
            <div class="card-body">
                <form action="{{ route('anak.update', $anak->id) }}" method="POST">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select class="form-control @error('ibu_id') is-invalid @enderror" id="ibu_id"
                            ...
                            <option {{ old('ibu_id', $anak->ibu_id) == $orangTua->id ? 'selected' : '' }} value="{{ $orangTua->id }}">{{ $orangTua->nama }}</option>
                            @endforeach
                        </select>
                        @error('ibu_id')
                    ...
                    <div class="mb-3">
                        ...
                        <select id="rentang_umur" name="rentang_umur" class="form-control" required>
                            <option value="6-11 bulan" {{ $anak->rentang_umur == '6-11 bulan' ? 'selected' : '' }}>6-11 bulan</option>
                            <option value="1-3 tahun" {{ $anak->rentang_umur == '1-3 tahun' ? 'selected' : '' }}>1-3 tahun</option>
                            <option value="4-6 tahun" {{ $anak->rentang_umur == '4-6 tahun' ? 'selected' : '' }}>4-6 tahun</option>
                        </select>
                    </div>
                    <div class="mb-3">
                        <label for="detail_umur" class="form-label">Detail Umur</label>
                        <input type="text" class="form-control" name="detail_umur" id="detail_umur" value="{{ $anak->detail_umur }}" required>
                    </div>
